...
export function RevenuePage() {
  ...
  const monthlyAverage = branch.monthly.length ? ytdTotal / branch.monthly.length : 0;
  ...
  return (
    <div className="fade-in space-y-3.5">
      <PageHeader
        title={lang === "th" ? "ภาพรวมยอดขาย & การเติบโต" : "Sales performance - recovery view"}
        sub={
          lang === "th"
        ...
        right={
          <>
            ...
            <Button size="sm" variant="outline" className="gap-2" onClick={handleExport}>
              ...
              {exported ? (lang === "th" ? "ส่งออกแล้ว" : "Exported") : t.common.export}
            </Button>
          </>
        }
      />
          ...
          <CardContent className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
            <div className="text-[13px]">
              <span className="font-semibold">
                {lang === "th" ? "ยังไม่มีข้อมูลรายได้จาก backend" : "No backend revenue data"}
              </span>
        ...
        <Card className="rounded-[10px] border border-border bg-card shadow-none">
          <CardContent className="px-6 py-10 text-center">
            ...
            <div className="mt-3 text-[15px] font-semibold">
              {lang === "th" ? "ไม่มีข้อมูลสำหรับหน้ารายได้" : "No data for the revenue page"}
            </div>
        ...
        <Card className="rounded-[10px] border border-border bg-card shadow-none">
          <CardContent className="space-y-4 p-4.5">
            <TargetMetric
              ...
              progress={monthlyAverage > 0 ? latestMonth / monthlyAverage : 0}
              note={
                lang === "th"
      ...
      <XSectionHeader
        idx="03"
        title={lang === "th" ? "หมวดสินค้า · ฉุดยอดและขับยอด" : "Categories · drag and lift"}
        sub={
          lang === "th"
      ...
      <Card className="rounded-[10px] border border-border bg-card shadow-none">
        <CardContent className="space-y-3 p-4.5">
          {categoryPacing.map((category) => (
            <CategoryTargetRow key={category.en} category={category} lang={lang} />
          ))}
        </CardContent>
      </Card>
      ...
      <section className="grid grid-cols-1 gap-3.5 xl:grid-cols-2">
        <Card className="rounded-[10px] border border-border bg-card shadow-none">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              {lang === "th" ? "สัดส่วนรายได้วันนี้" : "Today revenue share"}
            </CardDescription>
          </CardHeader>
        ...
        <Card className="rounded-[10px] border border-border bg-card shadow-none">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              {lang === "th" ? "วิธีชำระเงินที่ลูกค้าเลือก" : "Customer payment mix"}
            </CardDescription>
          </CardHeader>
      ...
      <section className="grid grid-cols-1 gap-3.5 md:grid-cols-3">
        <CustomerMetricCard
          ...
          target={comparisonBasket > 0 ? fmtMoney(comparisonBasket) : undefined}
          progress={comparisonBasket > 0 ? avgBasket / comparisonBasket : undefined}
          note={lang === "th" ? "คำนวณจากยอดขายรวม / จำนวนรายการประมาณการ" : "Calculated from total sales / estimated transactions"}
        />
        <CustomerMetricCard
          icon={Users}
          label={lang === "th" ? "จำนวนรายการ" : "Transactions"}
          value={fmtNum(transactions)}
          target={comparisonTransactions > 0 ? fmtNum(comparisonTransactions) : undefined}
          progress={
            comparisonTransactions > 0 ? transactions / comparisonTransactions : undefined
          }
          note={lang === "th" ? "ประมาณจากยอดขาย / 480 บาทต่อบิล" : "Estimated from sales / ฿480 per basket"}
        />
        <CustomerMetricCard
          icon={WalletCards}
          label={lang === "th" ? "การเปลี่ยนแปลงบิลเฉลี่ย" : "Basket change"}
...
function getRevenueSnapshot(branch: ReturnType<typeof useBranchData>["data"], range: Range) {
  if (range === "day") {
    return {
      ...
      description: { th: "รายชั่วโมงจาก backend", en: "Hourly series from backend" },
    };
  }
  ...
  if (range === "year") {
    ...
    return {
      ...
      labels: branch.monthly.map((_, index) => String(index + 1)).slice(latest.length ? 1 : 0),
      label: { th: "YTD", en: "YTD" },
      description: { th: "รายเดือนจาก backend", en: "Monthly series from backend" },
    };
  }
  ...
  return {
    ...
    label: { th: range === "week" ? "สัปดาห์นี้" : "MTD", en: range === "week" ? "This week" : "MTD" },
...
function toCategoryPacing(category: Category): CategoryPacing {
  ...
  const comparison =
    trendRatio <= -0.99 ? 0 : category.v / (1 + trendRatio);
...
function GapHero({
  ...
  const fill =
    comparison > 0 ? Math.min(100, Math.max(6, (actual / comparison) * 100)) : 0;
  ...
  return (
    <div>
      ...
      <div className="mt-1 flex items-baseline gap-2">
        <span className="num text-[22px] font-bold tracking-tight">{value}</span>
  ...
  return (
    <div className="grid gap-2 md:grid-cols-[200px_1fr_150px_110px] md:items-center md:gap-3.5">
      <div className="flex min-w-0 items-center gap-2">
        {isDrag && <span className="size-1.5 shrink-0 rounded-full bg-destructive" />}
        <span className="truncate text-[13px] font-medium">{category[lang]}</span>
      </div>
      ...
      <div className="text-right">
        <Badge variant={isDrag ? "destructive" : "default"} className="gap-1">
          {isDrag ? <ArrowDown className="size-3" /> : <ArrowUp className="size-3" />}
    ...
    <div>
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-muted">
        <div
          ...
          style={{ width: `${category.share * 100}%`, background: category.color }}
        />
      </div>
    </div>
  );
      ...
      <CardContent className="p-4.5">
        ...
        {typeof delta === "number" && (
          <div className="mt-3">
            ...
            <span className="ml-2 text-xs text-muted-foreground">vs prev period</span>
          </div>
        )}
        <div className="mt-3 text-[12px] leading-5 text-muted-foreground">{note}</div>
      </CardContent>
    </Card>
  );
}
...
function InsightNote({
  ...
  return (
    <div className={`rounded-lg border border-primary/25 bg-primary-50/70 p-3 text-[12.5px] leading-5 ${className ?? ""}`}>
    ...
    <Card className="rounded-[10px] border border-primary/25 bg-primary-50/55 shadow-none">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-[13.5px]">
          ...
          {lang === "th" ? "Action เพื่อปิดช่องว่าง" : "Actions to close the gap"}
        </CardTitle>
        <CardDescription className="text-xs">
          {lang === "th"
